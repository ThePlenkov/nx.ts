...
`.husky/pre-commit` is a shell script (`#!/usr/bin/env sh`) that executes `bunx biome format --write --no-errors-on-unmatched` from the shell environment, followed by `git add -u` to stage the formatted changes. Because biome is installed at root and the hook runs from repo root, this works without `cd`.
