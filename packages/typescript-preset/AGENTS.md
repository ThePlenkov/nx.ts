...
| `build` | (`tsdown.config.*` or publishable `package.json` + `src/index.ts`) + `tsdown: true` | `npx tsdown`, outputs `{projectRoot}/dist`, `dependsOn: ['^build']`. Publishable = `exports`/`bin`, or `main` + `files`. |
